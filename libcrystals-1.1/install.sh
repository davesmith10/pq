#!/usr/bin/env bash
# install.sh — build and install libcrystals-1.1 to a system prefix.
#
# Creates a fat static archive (libcrystals-1.1.a) that bundles the Kyber, Dilithium,
...
echo "=== libcrystals-1.1 installer ==="
...
if [[ ${SKIP_BUILD} -eq 0 ]]; then
    echo "[1/5] Building libcrystals-1.1..."
...
# Fat static archive
install -m 644 "${FAT_ARCHIVE}" "${PREFIX}/lib/libcrystals-1.1.a"
ranlib "${PREFIX}/lib/libcrystals-1.1.a"
...
# CrystalsConfig.cmake — generated by libcrystals-1.1/install.sh
...
if(NOT TARGET Crystals::crystals)
    ...
    set_target_properties(Crystals::crystals PROPERTIES
        IMPORTED_LOCATION             "\${_crystals_root}/lib/libcrystals-1.1.a"
...
Libs: -L\${libdir} -lcrystals-1.1 -lXKCP
